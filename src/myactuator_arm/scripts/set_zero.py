...
Usage:
    ...
    ros2 run myactuator_arm set_zero.py joint2
...
class SetZeroNode(Node):
    ...
    def timeout(self):
        if not self.received:
            ...
            self.get_logger().info('Make sure the robot is running (ros2 launch myactuator_arm free.launch.py)')
            rclpy.shutdown()
    ...
    def print_results(self):
        ...
        print("  config/ethercat/joint1_motor.yaml or joint2_motor.yaml")
        print("  In tpdo -> channels -> position line, add: offset: <value>")
        print("  Then rebuild: colcon build --packages-select myactuator_arm")
        print("="*60 + "\n")
...
def main():
    ...
    if not args:
        print("Usage:")
        print("  ros2 run myactuator_arm set_zero.py <joint_name>")
        print("  ros2 run myactuator_arm set_zero.py --all")
        print("\nExamples:")
        print("  ros2 run myactuator_arm set_zero.py joint1")
        print("  ros2 run myactuator_arm set_zero.py joint2")
        print("  ros2 run myactuator_arm set_zero.py --all")
        return
